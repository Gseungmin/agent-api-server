...
public class MemberService {
    ...
    /* 카카오 회원저장 - 초대코드 */
    @Transactional
    public Member createMemberWithCode(Family family, MemberCreateRequestDto dto, String socialId) {
        Member member = new Member(dto, socialId);
        new Auth(member);
        family.addMember(member);
    ...
    /* 애플 회원저장 - 초대코드 */
    @Transactional
    public Member createAppleMemberWithCode(Family family, MemberAppleCreateRequestDto dto) {
        Member member = new Member(dto, dto.getUser());
        new Auth(member);
        family.addMember(member);
    ...
    /* 회원정보 업데이트 */
    public Member updateProfile(Member member, MemberUpdateRequestDto dto) {
        if (dto.getName() != null) {
