...
public class PageService {
    ...
    public List<CategoryResponseDto> getPageItemWithPost(BabyPage babyPage) {
        ...
        List<PostResponseDto> itemList = pageItemList.stream()
                .map(item -> new PostResponseDto(item.getPost()))
                .toList();
        ...
        return groupedByCategory.entrySet().stream()
                .map(entry -> new CategoryResponseDto(entry.getKey(), entry.getValue(), null, null))
                .toList();
    }
    ...
    public List<CategoryResponseDto> getPageItemWithNutrition(BabyPage babyPage) {
        ...
        List<NutritionResponseDto> itemList = pageItemList.stream()
                .map(item -> new NutritionResponseDto(item.getNutrition()))
                .toList();
        ...
        return groupedByCategory.entrySet().stream()
                .map(entry -> new CategoryResponseDto(entry.getKey(), null, entry.getValue(), null))
                .toList();
    }
    ...
    public List<CategoryResponseDto> getPageItemWithInspection(BabyPage babyPage) {
        ...
        List<InspectionResponseDto> itemList = inspectionItemList.stream()
                .map(item -> new InspectionResponseDto(item.getInspection()))
                .toList();
        ...
        return groupedByCategory.entrySet().stream()
                .map(entry -> new CategoryResponseDto(entry.getKey(), null, null, entry.getValue()))
                .toList();
    }
